...
router.get('/events', (req, res, next) => {
	if (req.query.start && req.query.end && moment(req.query.start, 'YYYY-M-DD', true).isValid() && moment(req.query.end, 'YYYY-M-DD', true).isValid()) {
		models.sequelize.query("SELECT cm.*, c.name AS `course_name` FROM (SELECT id, course_id FROM class WHERE deleted = 0 AND id in (SELECT class_id FROM `class_enrollment` WHERE deleted = 0 AND student_id = ?) AND part_of_term_id IN (SELECT id FROM part_of_term WHERE deleted = 0 AND term_id = (SELECT id FROM term WHERE (? BETWEEN start_date AND end_date) AND deleted = 0 ORDER BY start_date DESC LIMIT 1))) a INNER JOIN class_meeting cm ON a.id = cm.class_id LEFT JOIN course c ON a.course_id = c.id", {
			...
			for (let i = 0; i < values.length; i++) {
				...
				rule.all((date, index) => {
					return events.push({
						...
						overlap: true,
						url: '/student/classes/class/' + values[i].class_id,
						color: eventColor
					});
				});
			}
