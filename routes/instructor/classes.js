...
router.get('/term/:termId/class/:classId', (req, res, next) => {
	Promise.all([
		models.sequelize.query("SELECT a.id AS `class_id`, a.section, a.max_enrollment, (SELECT COUNT(id) FROM class_enrollment WHERE class_id = ?) AS enrollment_count, c.number AS `course_number`, c.name AS `course_name`, c.description AS `course_description`, g.code AS `grade_mode_code`, g.name AS `grade_mode_name`, st.code AS `schedule_type_code`, st.name AS `schedule_type_name`, s.code AS `subject_code`, s.name AS `subject_name`, sc.name AS `school_name`, sc.code AS `school_code`, CONCAT(t.name, ' - ', pot.name) AS `term_name`, pot.start_date, pot.end_date FROM (SELECT * FROM class WHERE id = ? AND instructor_id = ?) a LEFT JOIN course c ON a.course_id = c.id LEFT JOIN grade_mode g ON c.grade_mode_id = g.id LEFT JOIN schedule_type st ON c.schedule_type_id = st.id LEFT JOIN subject s ON c.subject_id = s.id LEFT JOIN school sc ON c.school_id = sc.id LEFT JOIN part_of_term pot ON a.part_of_term_id = pot.id LEFT JOIN term t ON pot.term_id = t.id", {
			replacements: [req.params.classId, req.params.classId, req.user.id],
